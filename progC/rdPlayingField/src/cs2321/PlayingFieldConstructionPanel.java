 ...
 * 
 */
public class PlayingFieldConstructionPanel extends JPanel implements MouseListener, MouseMotionListener, Runnable{
	...
	public void paintComponent(Graphics g){
		...
		if (edit[SELECT_END]){
			pictures.get(ImageLoader.RARE_CANDY).draw(g, mouseX-37, mouseY-35);
		}
		
		g.setColor(Color.black);
		g.drawString(message, 800, 645);
	...
	public Pokemon findPokemon(int x, int y){
		...
		for (Pokemon poke : pokemon){
			if (Math.abs(poke.getCoordinates()[0]-x) <= 60 && poke.getCoordinates()[1]-y <= 90 && poke.getCoordinates()[1]-y >=0){
				return poke;
			}
		}
	...
	public void reset(){
		for (int i = 0; i < City.ARRAY_SIZE; i++){
			for (City city : graph.vertices()){
