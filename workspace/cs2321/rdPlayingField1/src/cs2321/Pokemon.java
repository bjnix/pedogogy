...
public class Pokemon implements Runnable, Infoable {
	...
	public Pokemon(PokemonGraph g, long speed, int searchNum, Picture p, String id, int x, int y, int power){
	...
	public void drawStats(Graphics g, int x, int y){
		...
		g.drawString("Power : " + power, x, y+15);
	}
	...
	public void battle(Pokemon enemy){
		...
		Pokemon loser = (enemy.getPower()>power)? this : enemy;
		Pokemon winner = (enemy.getPower()<power)? this : enemy;
		try {
			loser.die(winner);
		} catch (InterruptedException e) {
			e.printStackTrace();
