...
public class HashMap<K, V> implements Map<K, V> {
	...
	public V get(K key) throws InvalidKeyException {
		//TCJ: Worst case is if every entry were to hash into one bucket.
		//Expected case would be constant because the hash code enables
		//constant time access
		if( key == null){ throw new InvalidKeyException("Invalid Key for get(K,V)");}
		Integer c = key.hashCode()%mNumberOfBuckets;
		if ( c < 0)
		{
			c = c + mNumberOfBuckets;
		}
	...
	public Iterable<K> keys() {
		//TCJ: Because the iterator has to go through the list and then through the bucket, but it 
		//stays linear because it does not go n * n times. n is the 
		//number of elements in the bucket.
	...
	public V remove(K key) throws InvalidKeyException {
		...
		//Expected case would be constant because the hash code enables
		//constant time access
		if( key == null){ throw new InvalidKeyException("Invalid Key for remove(K,V)");}
		Integer c = key.hashCode()%mNumberOfBuckets;
		if ( c < 0)
		{
			c = c + mNumberOfBuckets;
