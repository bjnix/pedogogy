...
whileExpr(whAttr) ::= expr(exAttr).
		{
			...
			whAttr->symIndex =
			emitWhileLoopTest(instList,lastSymtab(symtabStack),exAttr->symIndex);
			free(exAttr);
		}
...
variable(varAttr) ::= IDENTIFIER(idAttr).
		{
			...
			SymTable symtab_current = findSymtab(symtabStack,idAttr->name);
			int symIndex = SymQueryIndex(symtab_current,idAttr->name);
			varAttr->symIndex =emitComputeVariableAddress(instList,symtab_current,symtab,symIndex);
			free(idAttr);
		}
...
variable(varAttr) ::= IDENTIFIER(idAttr) LBRACKET expr(exAttr) RBRACKET.
			...
			int symIndex = SymQueryIndex(symtab_current,idAttr->name);
			varAttr->symIndex =
			emitComputeArrayAddress(instList,symtab_current,symIndex,symtab,exAttr->symIndex);
			free(idAttr);
			free(exAttr);
               	}
