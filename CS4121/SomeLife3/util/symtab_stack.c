...
SymTable findSymtab(SymtabStack stack, char* key) {
	...
	if (SymQueryIndex(currentSymtab(stack),key) != SYM_INVALID_INDEX)
	{
		return currentSymtab(stack);
	}
	else if(SymQueryIndex(lastSymtab(stack),key) != SYM_INVALID_INDEX)
	{
		return lastSymtab(stack);
	}else{
		return NULL;
	}
