 ...
 * codegen.c
 *
 * S set of routines to handle the code generation in the SomeLife Compiler
 */
 ...
 * emitWhileBegin
 *
 * Add label to beginning of loop 
 *
 * @param instList
 * @prarm brNum
 */
void emitWhileBegin(Dlist instList, int brNum)
{
	char* brLabel = (char*)malloc(sizeof(char)*15);
	snprintf(brLabel,14,"LOOP_%d", brNum);
	char* inst = nssave(2, brLabel, ":");
	//push brLabel number onto stack
	dlinkPush(dLinkNodeAlloc(brNum),instList);
	dlinkAppend(instList,dlinkNodeAlloc(inst));
}
 ...
 * emitWhileEnd
 * 
 * Add label to the end of a loop 
 *
 * @param instList
 */
void emitWhileEnd(Dlist instList)
{
	char* brLabel = (char*)malloc(sizeof(char)*15);
	int brNum = (int)(dlinkPop(instList->atom));
	snprintf(brLabel,14,"LOOP_%d", brNum);
	char* inst = nssave(2, "b", brLabel);
	dlinkAppend(instList,dlinkNodeAlloc(inst));
	inst = nssave(2, brLabel, "_END:");
	dlinkAppend(instList,dlinkNodeAlloc(inst));
}
 ...
 *
 */
void emitWhileBoolCheck(Dlist instList, Symtable symtab, int boolIndex)
{
	char* brLabel = (char*)malloc(sizeof(char)*15);
	char* rT = (char*)SymGetFieldByIndex(symtab,boolIndex,SYM_NAME_FIELD);
	int brNum = (int)(dlinkPop(instList->atom));
	
	snprintf(brLabel,14,"lOOP_%d", brNum);
	char* inst = nssave(5 ,"\tbeq $s0,",rT,",",brLabel,"_END:");
 ...
 * emitBranch
 *
 * adds branch command for "if" statement
 *
 * @param 
 */

void emitBranch(DList instList, SymTable symtab, int brNum, int boolIndex)
{
	char* brLabel = (char*)malloc(sizeof(char)*15);
	char* rT = (char*)SymGetFieldByIndex(symtab,boolIndex,SYM_NAME_FIELD);


	snprintf(brLabel,14,"BR_%d",brNum);
	char* inst = nssave(4,"\tbeq $s0,",rT,",",brLabel)
 ...
 * emitIf
 *
 * adds branch label for "if" statement 
 *
 */
void emitIfThen(Dlist instList,SymTable symtab, int brNum)
{
	char* brLabel = (char*)malloc(sizeof(char)*15);
 ...
 * emitElse
 * 
 * add "else" branching command
 *
 * @param
 */
 void emitElseThen(Dlist instList,Symtable symtab, int brNum)
 {
 	char* brLabel = (char*)malloc(sizeof(char)*15);

 	snprintf(brLabel,14,"BR_%d",brNum);
 	char* inst = nssave(3,"\tb",brLabel,"_END");
  ...
  * emitElseEnd
  *
  * add "else" branching destination
  *
  * @param
  */
  void emitElseThenEnd(Dlist instList, int brNum)
  {
  	char* brLabel = (char*)malloc(sizeof(char)*15);
  	int brNum = (int)(dlinkPop(instList->atom));

  	snprintf(brLabel,14,"BR_%d",brNum);
  	char* inst = nssave(2,brLabel,"_END:")
  }
