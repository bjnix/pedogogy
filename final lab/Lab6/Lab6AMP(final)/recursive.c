...
{			
	...
	*output = 0x00000000;		//initialize to 0
	
	__builtin_wrctl(0,0x1);
	__builtin_wrctl(3,0x2);
	//__builtin_stwio(input+8, 0x7);
	asm(
	 "mov r6, %[UART]\n\t"		//move addr of UART into r5
	 :
	 :[UART]"r"(input)
	 : "r8","memory"
	);
